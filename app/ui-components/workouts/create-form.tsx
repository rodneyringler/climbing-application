...
import { createWorkout } from '@/app/lib/workout/workout-actions';
import { Button } from '@/app/ui-components/button';
import { Program } from '@/app/lib/program/program';
import { useState, useEffect } from 'react';

export default function CreateWorkoutForm({ programs }: { programs: { name: string; description: string }[] }) {
  const [selectedProgram, setSelectedProgram] = useState('');
  const [workoutName, setWorkoutName] = useState('');
  const [startedTime, setStartedTime] = useState('');
  const [endedTime, setEndedTime] = useState('');

  // Auto-generate workout name when program is selected
  useEffect(() => {
    if (selectedProgram) {
      const program = programs.find(p => p.name === selectedProgram);
      if (program) {
        const currentDate = new Date().toLocaleDateString();
        setWorkoutName(`${program.name} - ${currentDate}`);
      }
    }
